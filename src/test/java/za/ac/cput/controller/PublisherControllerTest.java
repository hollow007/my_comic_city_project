...
class PublisherControllerTest {
    ...
    @BeforeEach
    void setUp() {
        publisher3 = PublisherFactory.buildPublisher(12345, "Lindo Publishers", 2006);
    ...
    @Test
    void b_read() {
        String url = base_url + "/read/" + 12345;
    ...
    void c_update() {
        ...
        Publisher newPublisher = new Publisher.PublisherBuilder().copy(publisher3)
                .setName("Lindokuhle Publishers")
                .build();
        ResponseEntity<Publisher> postResponse = restTemplate.postForEntity(url, newPublisher, Publisher.class);
        assertNotNull(postResponse);
        assertNotNull(postResponse.getBody());

        Publisher updatedPublisher = postResponse.getBody();
        assertEquals(newPublisher.getPublisherId(), updatedPublisher.getPublisherId());
